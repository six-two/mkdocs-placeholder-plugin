...
DEFAULT_CSS_PATH = "assets/stylesheets/placeholders.css"
...
def main():
    ...
    ap.add_argument("directory", help="directory containing your HTML (or with '--phase markdown' your Markdown) files")
    ...
    ap.add_argument("--css-path", default=DEFAULT_CSS_PATH, help=f"the path where the CSS file will be written to if --copy-resources is used (default: {DEFAULT_CSS_PATH})")
    ...
    ap.add_argument("--phase", choices=["markdown", "html", "both"], default="both", help="what phase(s) to run. For an explanation check the website (default: 'both')")
    ...
    docs_dir = args.directory
    for dirpath, _dir_names, file_names in os.walk(docs_dir):
...
def copy_assets(args, placeholder_config):
    if args.css_path:
        ...
        _write_to_file(args.directory, args.css_path, css_text, "a")
    ...
    output_directory = os.path.join(args.directory, args.js_dir)
