...
class PageProcessor:
    def __init__(self, config: PlaceholderConfig) -> None:
        ...
        if self.config.settings.create_no_js_fallback:
            ...
            self.input_tag_modifier = StaticInputElementReplacer(config.placeholders, add_line_in_warning)
        else:
            self.input_tag_modifier = NormalHtmlInputElementHandler(config.placeholders, add_line_in_warning)
